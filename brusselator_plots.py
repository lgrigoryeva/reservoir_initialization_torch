...
V, D, eps, x_chunks_train, c_chunks_train, interp_c = \
    create_geometric_harmonics(dataset_train, dataset_test, config, model)
...
trajectory_w_warmup_initial, _ = model.integrate(initial_condition[start_point - warmup_length : start_point],
                                         len(dataset_test.tt) - start_point - 1)
...
initial_test_chunk = dataset_test.input_data[
    idx,
    config["DATA"]["max_warmup"] - config["GH"]["gh_lenght_chunks"] : config["DATA"]["max_warmup"]]
...
trajectory_w_warmup, _ = model.integrate(torch.tensor(initial_test_chunk),
                                         len(dataset_test.tt)-config["DATA"]["max_warmup"]-1)
...
ax.plot(dataset_test.tt[config["DATA"]["max_warmup"]-config["GH"]["gh_lenght_chunks"]:],
        trajectory_w_warmup, label='prediction with warmup')
ax.plot(dataset_test.tt[config["DATA"]["max_warmup"]-config["GH"]["gh_lenght_chunks"]:],
        trajectory_gh, label='prediction with GH')
ax.set_xlabel('')
ax.set_ylabel('')
# plt.savefig('')
plt.legend()
...
ax4.plot(dataset_test.tt[:config["DATA"]["max_warmup"]], dataset_test.input_data[idx, :config["DATA"]["max_warmup"]],
         '-x', markersize=3, label="Driven", color='green')
...
ax5.plot(dataset_test.tt[config["DATA"]["max_warmup"]:-1],
         dataset_test.input_data[idx, config["DATA"]["max_warmup"]:], '-x', markersize=3, label="Actual", color='red')
ax5.plot(dataset_test.tt[config["DATA"]["max_warmup"]-config["GH"]["gh_lenght_chunks"]:],
         trajectory_w_warmup, '-x', markersize=3, label='Autonomous with 5 steps warmup', color='blue')
ax5.plot(dataset_test.tt[config["DATA"]["max_warmup"]-config["GH"]["gh_lenght_chunks"]:config["DATA"]["max_warmup"]],
         trajectory_w_warmup[:config["GH"]["gh_lenght_chunks"]], '-x', markersize=3, label='Driven', color='green')
ax5.plot(dataset_test.tt[config["DATA"]["max_warmup"]-config["GH"]["gh_lenght_chunks"]:],
         trajectory_gh, '-x', markersize=3, label='Autonomous with cold start', color='orange')
...
ax1.text(-0.23, 1.0, 'a', transform=ax1.transAxes,
         size=12, weight='bold')
ax2.text(-0.23, 1.0, 'b', transform=ax2.transAxes,
         size=12, weight='bold')
