...
from utils import ESN, ESNModel, LoadBrusselatorDataset, LorenzDataset, config, progress
...
def main():
    ...
    if config["EXAMPLE"] == "brusselator":
        dataset_train = LoadBrusselatorDataset(path=config["PATH"], filename="training_data.npz", verbose=True)
        ...
        dataset_train = LorenzDataset(
            config["DATA"]["n_train"],
            config["DATA"]["l_trajectories"],
            config["MODEL"]["input_size"],
            verbose=True,
        )
        dataset_val = LorenzDataset(
            config["DATA"]["n_val"],
            config["DATA"]["l_trajectories"],
            config["MODEL"]["input_size"],
            verbose=False,
        )
        dataset_test = LorenzDataset(
            config["DATA"]["n_val"],
            config["DATA"]["l_trajectories"],
            config["MODEL"]["input_size"],
            verbose=False,
    ...
    if not config["TRAINING"]["ridge"]:
        ...
        ax.plot(
            np.arange(config["TRAINING"]["epochs"]),
            val_loss_list,
    ...
    predictions, _ = model.integrate(
        torch.tensor(dataset_test.x[0][:warmup], dtype=torch.get_default_dtype()),
