...
from utils import LoadBrusselatorDataset, ESN, ESNModel, config, create_chunks, dmaps
...
def get_hidden_states(dataset, model):
    ...
    hidden_states = model.net.forward(torch.tensor(dataset.x,
...
def plot_integration_w_gh(dataset_test, path='data/'):
    ...
    ax.plot(np.append(tt_init[-warmup_length:], ttnow), trajectory_w_warmup, '.-', label='with warmup',
            color=COLORS[2])
    ax.plot(ttnow[:min(len(initial_condition[start_point:]), Ttotal)],
    ...
    ax.plot(ttnow[:len(trajectory)-warmup_length],
            ...
            label='with geometric harmonics', color=COLORS[7], markersize=5)
    ax.plot(tt_init[-warmup_length:], initial_condition[start_point-warmup_length:start_point],
            '.-', label='warmup', color=COLORS[1], markersize=5)
...
def main():
    ...
    x_chunks_train = create_chunks(x_data_train, config["DATA"]["max_n_transients"],
                                   config["DATA"]["gh_lenght_chunks"],
                                   config["DATA"]["shift_betw_chunks"])
    c_chunks_train = create_chunks(c_data_train, config["DATA"]["max_n_transients"],
                                   config["DATA"]["gh_lenght_chunks"],
                                   config["DATA"]["shift_betw_chunks"])
    c_chunks_train = c_chunks_train[:, 0, :]

    x_chunks_test = create_chunks(x_data_test, config["DATA"]["max_n_transients"],
                                  config["DATA"]["gh_lenght_chunks"],
                                  int(5*config["DATA"]["shift_betw_chunks"]))
    c_chunks_test = create_chunks(c_data_test, config["DATA"]["max_n_transients"],
                                  config["DATA"]["gh_lenght_chunks"],
                                  int(5*config["DATA"]["shift_betw_chunks"]))
    ...
    V_train, V_test, c_chunks_train_train, c_chunks_train_test = \
        train_test_split(V, c_chunks_train, random_state=np.random.seed(10), train_size=8/10)

    pw = pdist(V_train, "euclidean")
    eps_GH = np.median(pw**2)*0.05

    GH = diffusion_maps.SparseDiffusionMaps(points=V_train,
                                            epsilon=eps_GH,
    ...
    interp_c = diffusion_maps.GeometricHarmonicsInterpolator(
    ...
    plt.savefig(config["PATH"]+'geometric_harmonics_h_' +
                str(config["DATA"]["gh_lenght_chunks"])+'.png', dpi=300)
    plt.savefig(config["PATH"]+'geometric_harmonics_h_' +
    ...
    for i in tqdm(range(x_chunks_test.shape[0])):
        nystrom_data[i, :] = nystrom(x_chunks_test[i, :].reshape(1, -1),
                                     V, x_chunks_train, D, eps).T

    fig = plt.figure()  # figsize=(7, 7))
    # ax = fig.add_subplot(111)
    plt.scatter(V[:, 0], V[:, 1], c='k', s=5)
    plt.scatter(nystrom_data[:, 0], nystrom_data[:, 1],
                c=COLORS[1], marker='x', s=2, label='Restricted with Nyström')
    plt.legend()
    plt.xlabel(r'$\phi_1$')
    plt.ylabel(r'$\phi_2$')
    plt.savefig(config["PATH"]+'nystrom_test_' +
                str(config["DATA"]["gh_lenght_chunks"])+'.png', dpi=300)
    ...
    initial_test_chunk = dataset_test.x[idx, config["DATA"]["max_warmup"] -
                                        config["DATA"]["gh_lenght_chunks"]:config["DATA"]["max_warmup"]]
    nystrom_test_chunk = nystrom(initial_test_chunk.reshape(1, -1), V, x_chunks_train, D, eps).T
    h_pred = interp_c(nystrom_test_chunk)
    print(c_data_test[idx, config["DATA"]["max_warmup"]-config["DATA"]["initial_set_off"] -
